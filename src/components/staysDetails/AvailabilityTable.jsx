            ...
            {rooms.map((room) => (
              <tr key={room.id} className="availability__room-row">
                <td className="availability__room-cell">
                  ...
                  <div className="availability__room-bed">{room.bed}</div>
                  <div className="availability__room-features">
                    {room.features.map((f) => (
                      <span key={f} className="availability__feature-tag">{f}</span>
                    ))}
                  </div>
                </td>
                ...
                <td className="availability__price-cell">
                  <div className="availability__original-price">{room.originalPrice}</div>
                  <div className="availability__current-price">{room.currentPrice}</div>
                  <div className="availability__price-note">Includes taxes and charges</div>
                  <span className="availability__badge availability__badge--discount">{room.discount}</span>
                  <span className="availability__badge availability__badge--deal">{room.deal}</span>
                </td>
                <td className="availability__choices-cell">
                  {room.choices.map((choice, i) => (
                    <div key={i} className="availability__choice">
                      ...
                      {choice.includes("left") && <span style={{ color: "#e25c5c" }}>•</span>}
                      <span>{choice}</span>
                    </div>
                  ))}
                </td>
                ...
                <td>
                  <div className="availability__reserve-col">
                    <button className="availability__reserve-btn">I'll reserve</button>
