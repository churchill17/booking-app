    ...
    <div className="admin-payment-page">
      <div className="admin-page-header-row">
        <div>
          ...
          <p className="admin-page-subtitle">Track and manage all host bookings</p>
        </div>
        <button className="admin-btn-primary" type="button" onClick={onRefresh}>
          Refresh bookings
        </button>
      </div>
      ...
      <div className="admin-payment-summary">
        {summaries.map((s, i) => (
          <div className="admin-pay-summary-card" key={i}>
            <div className="admin-pay-icon" style={{ background: `${s.color}18` }}>
              <span>{s.icon}</span>
            </div>
            <div>
              <p className="admin-pay-label">{s.label}</p>
              <p className="admin-pay-value" style={{ color: s.color }}>
                {s.value}
              </p>
            </div>
          </div>
      ...
      <div className="admin-payment-card">
        ...
        {error ? <div className="admin-empty-state">{error}</div> : null}
        ...
        <table className="admin-payment-table">
          ...
          <tbody>
            {filtered.map((booking) => (
              <tr key={booking.id}>
                <td className="admin-tenant-cell">
                  ...
                  <div>
                    ...
                    <div className="admin-text-muted">{booking.guestEmail}</div>
                  </div>
                </td>
                <td>
                  <div className="admin-property-booking-cell">
                    <img
                      className="admin-property-booking-image"
                      src={booking.propertyImage || PLACEHOLDER_IMAGE}
                      alt={booking.propertyName}
                    />
                    <div>
                      <div className="admin-property-booking-name">
                ...
                <td>
                  <AdminPayStatusBadge kind="booking" status={booking.status} />
                </td>
                <td>
                  <AdminPayStatusBadge
                    kind="payment"
...
function AdminPayStatusBadge({ status, kind }) {
