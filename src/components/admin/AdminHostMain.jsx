...
function normalizeAdminProperty(item) {
  ...
  return {
    ...
    // Host info from JOIN
    hostName: item?.hostName || item?.host_name || "",
    hostEmail: item?.hostEmail || item?.host_email || "",
      ...
      case "dashboard":
        return (
          <AdminDashboardPage
            ...
            isLoading={isLoading}
            error={error}
            onRefresh={() => Promise.all([loadListings(), loadDashboard(), loadBookings()])}
            dashboardHost={{ firstName: "Admin", lastName: "" }}
